import sqlite3
con = sqlite3.connect("Annual.db")

def insertData(Name,language1,language2,maths,science,social,total,average,Remarks):
    
    qry="insert into Annual(NAME,language1,language2,maths,science,social,total,average,Remarks) values (?,?,?,?,?,?,?,?,?);"
    
    con.execute(qry,(Name,int(language1),int(language2),int(maths),int(science),int(social),int(total),int(average),str(Remarks)))
    con.commit()
    print(" STUDENT Annual mark  added Sucessfully")
    
def updateData(Name,language1,language2,maths,science,social,total,average,Remarks,ID):
    qry="update Annual set NAME=?,language1=?,language2=?,maths=?,science=?,social=? ,total=? ,average=? ,Remarks=? where ID =?;"
    con.execute(qry,(Name,int(language1),int(language2),int(maths),int(science),int(social),int(total),int(average),str(Remarks),ID))
    con.commit()
    print(" STUDENT Annual mark  updated Sucessfully")   
    
def deleteData(ID):
    qry="delete from Annual where ID =?;"
    con.execute(qry,(ID))
    con.commit()
    print(" STUDENT Annual mark  Deleted Sucessfully")   


def selectData(ID):
    qry="select name,language1,language2,maths,science,social from Annual where ID=?;"
...
def Remarks():
    qry="select from Annual where language1>=35 and language2>=35 and maths>=35 and science>=35 and social>=35; " 
    qry="select from Annual where language1<35 or language2<35 or maths<35 or science<35 or social<35; "
