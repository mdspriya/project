import sqlite3
con = sqlite3.connect("studmark.db")

def insertData(NAME,language1,language2,maths,science,social,total,average,Remarks):
    
    qry="insert into studmark(NAME,language1,language2,maths,science,social,total,average,Remarks) values (?,?,?,?,?,?,?,?,?);"
...
def updateData(NAME,language1,language2,maths,science,social,total,average,Remarks,ID):
    qry="update studmark set NAME=?,language1=?,language2=?,maths=?,science=?,social=? ,total=? ,average=? ,Remarks=? where ID =?;"
...
def deleteData(ID):
    qry="delete from studmark where ID =?;"
    con.execute(qry,(ID))
    con.commit()
...
def selectData(ID):
    qry="select name,language1,language2,maths,science,social from studmark where ID=?;"
...
def Remarks():
    qry="select from studmark where language1>=35 and language2>=35 and maths>=35 and science>=35 and social>=35; " 
